...
struct UserStyle: Equatable, Sendable {
    ...
    func save() {
        UserDefaults.standard.set(["color": colorHex, "rainbow": rainbow, "gif": gifURL], forKey: Self.key)
    }
...
/// Renders a username in the user's chosen color, or as an animated rainbow.
    ...
    var body: some View {
        if style.rainbow {
            Text(name).font(font)
                ...
                .overlay {
                    LinearGradient(colors: [.red, .orange, .yellow, .green, .cyan, .blue, .purple, .red],
                                   startPoint: .leading, endPoint: .trailing)
                        .frame(width: 600)
                        .offset(x: phase)
                        .mask(Text(name).font(font))
                }
                .fixedSize()
                .onAppear {
                    withAnimation(.linear(duration: 3).repeatForever(autoreverses: false)) { phase = -300 }
                }
        } else {
            Text(name).font(font).foregroundStyle(style.color ?? fallback).fixedSize()
        }
    }
}
